    ...
    def action_confirm(self):
        ...
        for order in self:
            ...
            if warehouse.is_delivery_set_to_done and order.picking_ids:
                #Si el almacén tiene configurado completar las entregas automáticamente (is_delivery_set_to_done). Si hay al menos un albarán asociado con la orden de venta (order.picking_ids).
            ...
            if warehouse.validate_invoice and order.invoice_ids:
                #Si el almacén está configurado para validar facturas automáticamente (validate_invoice). Si hay facturas asociadas con la orden de venta.
